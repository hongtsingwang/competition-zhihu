...
for index, line in enumerate(open(train_input_file)):
    ...
    for single_char in char_list.split(","):
...
for key, value in output_char_cocurrence_file.iteritems():
    if value == 1:
        continue
    output_char_cocurrence_file.write("\t".join([key[0], key[1], str(word_counts[value])]))
