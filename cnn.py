...
# 数据集
data_file = os.path.join(transform_data_dir, "data_topic_block_0.txt")
tf.flags.DEFINE_string("data_file", data_file,
...
# 每一种卷积核个数
tf.flags.DEFINE_integer("num_filters", 1024,
                        "Number of filters per filter size (default: 1024)")
# dropout参数
tf.flags.DEFINE_float("dropout_keep_prob", 0.5,
                      "Dropout keep probability (default: 0.5)")
# l2正则化参数
tf.flags.DEFINE_float("l2_reg_lambda", 0.0005,
                      "L2 regularization lambda (default: 0.0005)")
...
tf.flags.DEFINE_integer("checkpoint_every", 200,
                        "Save model after this many steps (default: 200)")
# 保存多少个模型
...
Weights = tf.Variable(tf.random_uniform(
    [vocab_size, embedding_size], -1.0, 1.0), name="Weights")
# shape:[None, sequence_length, embedding_size]
embedded_chars = tf.nn.embedding_lookup(Weights, input_x)
# 添加一个维度，shape:[None, sequence_length, embedding_size, 1] TODO 搞明白为什么要添加一个维度
embedded_chars_expanded = tf.expand_dims(embedded_chars, -1)

# Create a convolution + maxpool layer for each filter size TODO 搞清楚strides到底是怎么回事
# TODO 这么重要的一部分至少应该能够徒手写出来把。
pooled_outputs = []
...
with tf.Session() as sess:
    ...
    for batch in batches:
        i = i + 1
        # 得到一个batch的数据
        # TODO 搞清楚为什么batch前面要加*号
        x_batch, y_batch = zip(*batch)
        # 优化模型
        sess.run([optimizer], feed_dict={
                 input_x: x_batch, input_y: y_batch, dropout_keep_prob: FLAGS.dropout_keep_prob})
